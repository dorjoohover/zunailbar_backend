...
exports.login = asyncHandler(async (req, res, next) => {
  ...
  if (user?.status === "9") {
    shareholder = await req.db.shareholder.findOne({
      where: {
  ...
  if (user?.status === "9" && !shareholder) {
    throw new MyError("main_error_", 400); //main_error_?????
  }
  ...
  // var updateShareholder = await req.db.shareholder.findByPk(shareholder?.id);
  // updateShareholder.userId = user.id;
  ...
  var cookieOptions = getCookieOptions();
  // successlog.info(`Login log: ${now()}, ${user}, ${shareholder}`);
  // start session
  // console.log("______________________ ХЭРЭГЛЭГЧ:::::", user);
  req.session.user = user;
  req.session.save();
  // console.log(
  //   " _ _ _ _ __ _ _ __ _ _ _ __ _ _ __ Хэрэглэгч логин хийлээ _ _ _ _ __ _ _ __ _ _ _ __ _ _ __ "
  // );
  ...
  //   },
  // });
  // frcLogStream(log_data);
  // logger.info(`Login log:,${log_data}`);
  // logger.error(`Login log:,${log_data}`);
});
...
exports.register = asyncHandler(async (req, res, next) => {
  ...
  const exUserEmail = await req.db.user.findOne({
    where: {
  ...
  if (exUserEmail) {
    ...
    throw new MyError("main_error_email_registered", 400); //email өмнө нь бүртгэлтэй байна
  }
  ...
  if (!exUser) {
    ...
    if (exUser.emongolId) {
      throw new MyError(
        // `${exUser.register} e-mongolia -аар бүртгүүлсэн байна, ${exUser.register} you have been registered by e-mongolia before`,
        `main_error_emongolia_registered`,
        400
      );
    } else
      throw new MyError(
        // `${exUser.register} тархалтын мэдээлэлд бүртгүүлсэн байна, ${exUser.register} has been registered before`,
        `main_error_register_registered`,
        400
      );
  }
